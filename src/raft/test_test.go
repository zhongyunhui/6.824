...
// The tester generously allows solutions to complete elections in one second  1秒钟内完成选举
...
////logEntries.  If there is a leader, that leader will fail quickly with a high
////probability (perhaps without committing the command), or crash after a while
////with low probability (most likey committing the command).  If the number of
////alive servers isn't enough to form a majority, perhaps start a new server.
////The leader in a new term may try to finish replicating logEntries entries that
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		if leader != -1 && (rand.Int()%1000) < int(RaftElectionTimeout/time.Millisecond)/2 {
			...
			connectArr := make([]bool, servers)
			for i:= 0; i < servers; i++ {
				connectArr[i] = cfg.connected[i]
			}
			DPrintf("现在的连接情况为%v", connectArr)
			nup -= 1
		}
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	// 客户端向raft service输入命令，leader通过start方法将command添加到logEntries中，通过传入的expected servers来判断是否全部应用完毕
	cfg.one(rand.Int(), servers, true)
	DPrintf("snapcommon开始")
	// 得到leader
	leader1 := cfg.checkOneLeader()
	...
	for i := 0; i < iters; i++ {
		...
		cfg.one(rand.Int(), servers-1, true)
		DPrintf("超过了snapshotInterval[%d]", SnapShotInterval)
		// 判断日志是否被压缩
